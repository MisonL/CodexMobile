...
该命令会先刷新 `dist/hf-space`，再在工作目录内初始化临时 git 仓库、提交生成产物并强制推送到目标 remote。`--force` 会覆盖目标 Space git 历史，仅用于首推或确认要重置生成目录时；目标 Space 可能已有协作者提交时不要使用。推送前必须先在 HuggingFace Space Settings 中配置 `CODEXMOBILE_RELAY_SECRET`。
