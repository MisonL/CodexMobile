...
当前 relay 支持小体积 JSON / text HTTP 请求、普通 `/ws` 事件转发，以及 `/api/uploads` 和 `/api/voice/transcribe` 的 chunked request streaming。`/ws/realtime`、生成图片二进制读取和大型响应下载仍然需要本地直连，relay 会返回明确错误。
